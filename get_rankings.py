...
def output_records(output_file, first_year, last_year):

    with open(output_file, 'wt') as fd:
        fd.write('<html>\n')
        fd.write('<body>\n')
        fd.write(f'<h1>Club records autogenerated from PowerOf10 {first_year} - {last_year}</h1>\n')
        for (event, _, _, _) in known_events: # debug: ['10K', 'HM', 'Mar', 'LJ', 'HepW', 'Dec']:
            ...
            for gender in ['W', 'M']:
                ...
                fd.write('<tr>\n')
                fd.write('<td><b>Rank</b></td><td><b>Performance</b></td><td><b>Athlete</b></td><td><b>Date</b></td><td><b>Fixture</b></td>\n')
                fd.write('</tr>\n')
...
def main(club_id=238, output_file='records.htm', first_year=2005, last_year=2023):
    ...
    output_records(output_file, first_year, last_year)
